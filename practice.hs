...
remove x (y:ys) = if x == fst y then ys
                                 ...
                                 -- General Expression And General Intpreter 


--gIntpreter :: Stmt -> State -> State
--gIntpreter (Assign x ) (State1 y)   = State1 ((remove )
...
remove2 x (State1 ((y,(A (IVal x1))):ys)) = if x == y then x1
                         else remove1 x (State1 ys)
remove2 x (State1 ((y,(B (BT x1))):ys)) = if x == y then x1
                         else remove1 x (State1 ys)
